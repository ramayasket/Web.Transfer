...
namespace Web.Transfer.Base32
{
    ...
    /// </summary>
    public sealed class Base32DecodePipe : IDisposable
    {
        private readonly Action<byte[]> _handler;

        private Base32Core.DecodingBlock _block;
        private readonly StringBuilder _builder = new StringBuilder();
        private readonly List<Base32Core.DecodingBlock> _accumulator = new List<Base32Core.DecodingBlock>();

        /// <summary>
        /// Initializes a new instance of the <seealso cref="Base32DecodePipe"/> using the specified decoded data handler.
        /// </summary>
        /// <param name="handler">A handler to receive decoded data.</param>
        public Base32DecodePipe(Action<byte[]> handler)
        {
            _handler = handler;
            _block = new Base32Core.DecodingBlock();
        }
